...
Functype funcTable[TABLESIZE]; /*hash size=2^14*/
...
/*Expressions*/
Type Exp(Node *n, Operand place) {//printName(n->name);
    Node *child = n->children;
    if (strcmp(child->name, "Exp") == 0) {
        ...
        if (strcmp(child2->name, "ASSIGNOP") == 0) {
            ...
            if (typeEqual(leftType, rightType)) {
                //print code here
                if (rightOp->kind == TEMPVAR && rightOp->u.var_no == temp_no &&
                    (leftOp->kind == TEMPVAR || leftOp->kind == VARIABLE))
        ...
        } else if (strcmp(child2->name, "AND") == 0 || strcmp(child2->name, "OR") == 0 ||
                   strcmp(child2->name, "RELOP") == 0)//and or /
        {
            //new temp
            Operand lb1 = malloc(sizeof(struct Operand_));
            lb1->kind = LABEL;
...
//exp condition
Type Exp_Cond(Node *n, Operand label_true, Operand label_false) {//printName(n->name);
    Node *child = n->children;
    Type type;
    if (strcmp(child->name, "Exp") == 0) {
        Node *child2 = child->next;
        if (strcmp(child2->name, "RELOP") == 0)//< >
        {
            ...
            else if ((tp->kind == 0 || tp->kind == 3) && (tp2->kind == 0 || tp2->kind == 3) &&
                     tp->u.basic == tp2->u.basic) {
...
bool Args(Node *n, FieldList f, Operand arg_list) {//printName(n->name);
